...
## The proprietary reality: shapefiles

::::: columns
::: {.column width="70%"}
Both the geometric information and attribute table could be saved within one file, often as *ESRI Shapefiles*. Shapefiles consist of at least three mandatory files with the following extensions:
...
You don't have to remember what they stand for, but you need all of those files.
::: 

::: {.column width="30%"}
![](../img/shapefile_attached.png){fig-align="center"}
::: 
:::::
...
## A common pitfall: mismatched CRS

`R` won't stop you from plotting two layers with **different CRS** together — it just uses the raw coordinate values, wherever they happen to fall.

```{r}
#| output-location: fragment
# Cologne Cathedral, given in geographic coordinates (lon/lat, WGS84)
cologne_point <- sf::st_sfc(
  sf::st_point(c(6.9603, 50.9375)),
  crs = 4326
)
...
## Where did Cologne go? 

The point exists at coordinates (6.96, 50.94). But `german_states` is projected in EPSG:25832, so the long/lat (6.96, 50.94) sits nowhere near the plotted extent.

```{r}
#| fig.asp: .8
#| output-location: column-fragment
plot(sf::st_geometry(german_states))
plot(cologne_point, add = TRUE, col = "red", pch = 19, cex = 2)
```
...
#| fig.asp: .8
#| output-location: column-fragment
cologne_point_aligned <- sf::st_transform(
  cologne_point,
  sf::st_crs(german_states)
)

plot(sf::st_geometry(german_states))
plot(cologne_point_aligned, add = TRUE, col = "red", pch = 19, cex = 2)
```

## Checklist: after importing vector data, always check...

::: {.incremental}
- **`sf::st_crs()`**: is a CRS defined at all? Does it match your other layers?
- **`sf::st_geometry_type()`**: points, lines, or polygons, as expected?
- **`sf::st_bbox()`**: does the extent make sense? 
- **`sf::st_is_valid()`** / **`sf::st_make_valid()`**: are geometries topologically valid? 
- **plot**: does it look right?
:::

## Exercise 2_1: Import Vector Data💪 {.center style="text-align: center;"}

🖱[Click here for the exercise](https://denabel.github.io/BIGSSS_geo_2026/exercises/2_1_Import_Vector_Data.html)
...
🖱 [Click here for the exercise](https://denabel.github.io/BIGSSS_geo_2026/exercises/2_2_Basic_Raster_Operations.html)
