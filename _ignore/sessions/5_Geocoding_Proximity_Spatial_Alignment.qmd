...
ggplot2::ggplot() +
  ...
  ggplot2::labs(
    title    = "Each piece = one ZIP × municipality overlap",
    subtitle = glue::glue("{nrow(zip_sub)} ZIPs → {nrow(intersection)} intersection pieces")
  ) +
...
::: {.column width="50%"}
![](../img/areal_interpolation.png){fig-align="center" size="50%"}

:::
:::::

## Spatial Alignment: raster and vector
...
# load raster 
pop_raster <- terra::rast("./data/census22_population.tif") |>
  terra::subst(from = -9, to = NA)

# verify / align CRS 
zip_sub_projected <- zip_sub |>
  sf::st_transform(terra::crs(pop_raster)) 

# crop to extent of the zip_sub vector
pop_raster_cropped <- pop_raster |>
  terra::crop(terra::vect(zip_sub_projected))

# area-weighted extraction
zip_pop <- exactextractr::exact_extract(
  x           = pop_raster_cropped,
  y           = zip_sub_projected,
  # population is a stock — sum, not mean
  fun         = "sum",   
  # area-weighted: accounts for partial cell coverage
  weights     = "area", 
  # carries zip_code through
  append_cols = "zip_code"      
) 
...
::: {.column width="50%"}
![](../img/exactextract.svg){fig-align="center" width="56%"}
:::
::::

## Spatial alignement: raster and data
...
zip_pop_sf <- zip_sub_projected |>
  dplyr::left_join(zip_pop, by = "zip_code")

ggplot2::ggplot(zip_pop_sf) +
  ggplot2::aes(fill = sum) +
